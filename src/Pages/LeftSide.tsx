...
interface SelectedUserProps {
  setSelectedUserData: React.Dispatch<
    React.SetStateAction<SelectedUserData | null>
  >;
}

const LeftSide: React.FC<SelectedUserProps> = ({ setSelectedUserData }) => {
  const navigate = useNavigate();
  const { isAuth, user }: any = useSelector<any>(
    (state: RootState) => state.auth
  );

  console.log(isAuth, user, "authuser");
  const [usersList, setUsersList] = useState<List[]>([]);
  const [allUsersList, setAllUsersList] = useState<List[]>([]);

  const getUsersList = async () => {
    const res = await axios.get(
      `http://localhost:3500/user/filterUsers/${user.id}`
    );
  ...
  //Slected user and store the selected user data in setSelectedUser
  const handleSelectedUser = (user: any) => {
    setSelectedUserData(user);
  };

  //Searh users filter onchange function
  const handleFilterUsers = (event: React.ChangeEvent<HTMLInputElement>) => {
    const searchValue = event.target.value.trim().toLowerCase();

    if (event.target.value) {
      const filterUserList = usersList.filter((item) =>
  ...
  return (
    <>
      <div className="w-[4%] p-2 flex flex-col justify-end">
        <button onClick={() => handleLogout()}>
          <BiLogOutCircle className="text-5xl p-2 hover:bg-gray-600 rounded-full duration-300" />
        </button>
      </div>
      <div className="w-[26%] bg-white p-4 border-2 border-r-slate-200 flex flex-col h-screen">
        <p className="text-xl font-bold">Hii {user.name}</p>
        ...
        <div className="mt-3">
          <input
            ...
            className="w-full border-2 border-b-slate-400 border-l-slate-100 border-r-slate-100 border-t-100 focus:border-b-slate-500 hover:border-b-slate-500 outline-none transition rounded-lg"
          />
        </div>
        {/* Search Input Box */}
        ...
        {/* Chat User List */}
        <div className="mt-5 overflow-y-scroll flex-grow scrollbar-thin scrollbar-thumb-slate-400 scrollbar-track-slate-200">
          {usersList.map((user, index) => (
            <div
              onClick={() => handleSelectedUser(user)}
              key={index}
              className="flex space-x-6 px-2 py-2 hover:bg-slate-200 duration-300 cursor-pointer"
            >
